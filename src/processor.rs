...
pub fn process_instruction(
    ...
    let instruction: StakeInstruction = StakeInstruction::unpack(instruction_data)?;
...
fn process_stake(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    account_data.token_account = nft_token_account.key.to_string();
    account_data.user_pubkey = user.key.to_string();
...
fn process_redeem(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    if *user.key.to_string() != account_data.user_pubkey {
        msg!("Incorrect stake account for user");
        return Err(StakeError::InvalidStakeAccount.into());
    }

    if *nft_token_account.key.to_string() != account_data.token_account {
